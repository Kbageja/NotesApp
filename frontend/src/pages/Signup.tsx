...
const SignUp: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.name.trim() && !isRegistered) newErrors.name = "Name is required"
    else if (formData.name.length < 2 && !isRegistered) newErrors.name = "Name must be at least 2 characters"

    if (!formData.email.trim()) newErrors.email = "Email is required"
    else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.email)) newErrors.email = "Enter a valid email"

    if (!formData.password && !needsVerification) newErrors.password = "Password is required"
    else if (formData.password.length < 8 && !needsVerification && !isRegistered)
      newErrors.password = "Password must be at least 8 characters"

    if (!formData.dateOfBirth && !isRegistered && !needsVerification)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData((prev) => ({ ...prev, password: originalPassword, dateOfBirth: "" }))
      handleSendOTP()
    } catch (error: any) {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      setIsLoading(false)
      toast.error(`Failed to start Google sign up: ${error.message || "Unknown error"}`)
    }
  }
    ...
    <div className="min-h-screen bg-white">
        ...
        <div className="flex flex-col min-h-screen">
          ...
          <div className="flex-1 px-6">
            <div className="mb-8">
              ...
              <p className="text-muted-foreground">Join HD Notes today</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="space-y-2">
                <Input
                  ...
                  className="h-12"
                  disabled={needsVerification || isRegistered}
                />
              ...
              <Button type="submit" size="lg" className="w-full h-12 bg-blue-500 text-white font-semibold rounded-xl" disabled={isLoading}>
                {isLoading ? (
            ...
            <div className="mt-6 text-center">
              ...
              <Link to="/signin" className="text-blue-500 underline font-medium hover:text-blue-600">
                Sign in
              </Link>
            </div>
      ...
      <div className="hidden md:flex max-h-[900px] mx-2">
        <div className="flex w-full bg-white rounded-3xl shadow-2xl overflow-hidden">
          {/* Left Panel */}
          <div className="w-2/5 px-16 my-3 flex flex-col justify-center bg-card">
            <div className="pb-12">
            ...
            <div className="mb-8">
              ...
              <p className="text-muted-foreground text-lg">Join HD Notes today</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-2">
              <div className="space-y-2">
                <Input
                  ...
                  disabled={needsVerification || isRegistered}
                />
                {errors.name && <p className="text-amber-500 font-semibold text-sm">{errors.name}!</p>}
              </div>
              ...
              <div className="space-y-2">
                <Input
                  ...
                  disabled={needsVerification || isRegistered}
                />
                {errors.dateOfBirth && <p className="text-amber-500 font-semibold text-sm">{errors.dateOfBirth}!</p>}
              </div>
